...
export class LoadNewComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (
      this.displayStockId !== null &&
      this.displayStockId !== undefined &&
      this.displayStockId !== true
    ) {
      ...
      if (this.productForm.valid) {
        if (
          ...
          const data = {
            quantity: Number(
              (
                ...
                Number(this.productForm.get('quantityToAdd')?.value)
              ).toFixed(3)
            ),
            price: this.productForm.get('price')?.value,
          };
          this.salesSvc
            .updateDisplayStock(data, this.displayStockId)
            .subscribe({
              error: (err: any) => {
              ...
              next: (resp: any) => {
                this.alertSvc.presentAlert(
                  'Éxito',
                  'Producto actualizado al punto de venta'
                );
          ...
          this.productForm
            .get('quantity')
            ?.setValue(
              Number(this.productForm.get('quantity')?.value.toFixed(3))
            );
          this.salesSvc.createDisplayStock(this.productForm.value).subscribe({
            error: (err: any) => {
              this.handleError(err);
            },
            next: (resp: any) => {
              this.alertSvc.presentAlert(
                'Éxito',
                'Producto cargado al punto de venta'
              );
              this.closeModal(value);
            },
          });
  ...
  getDisplayStock() {
    if (
      ...
      this.salesSvc.getDisplayStockById(this.displayStockId).subscribe({
        error: (err: any) => {
          this.handleError(err);
        },
        next: (resp: any) => {
          ...
          this.productForm
            .get('type_of_unit_measurement')
            ?.setValue(resp.type_of_unit_measurement.id);
  ...
  getProducts() {
    this.inventorySvc.getProducts(250, 0, true, '').subscribe({
      error: (err: any) => {
        this.handleError(err);
      },
      next: (resp: any) => {
    ...
    this.suggestions = this.products
      .filter(
        (product) =>
          ...
          product.code.toLowerCase().includes(event.query.toLowerCase())
      )
