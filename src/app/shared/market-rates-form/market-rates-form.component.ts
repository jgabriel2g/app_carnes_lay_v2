...
export class MarketRatesFormComponent  implements OnInit {
  ...
  constructor(private inventorySvc:InventoryService, private alertSvc: AlertsService, public authSvc:AuthService) { }
  ...
  sendMarketRate(){
    if (this.selectedProduct !== '' && this.description.length && this.price > 0 ) {
        this.formSubmit.emit({product:this.selectedProduct, description: this.description, price:this.price})
    } else {
